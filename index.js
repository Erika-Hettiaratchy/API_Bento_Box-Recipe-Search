...
export default function Home() {
  ...
  const [prepareWithin, setPrepareWithin] = useState(0);
  ...
  return (
    <div className="flex flex-col md:px-12 px-0 relative bg-background font-raleway items-center min-h-screen">
      <h1 className="text-6xl font-bold text-active mt-20">Bento Box</h1>
      <h2 className="text-primary text-2xl font-light mt-5">
        Search for recipes all over the world.
      </h2>
      <form
      ...
      >
        ...
        <div className="mt-5 flex sm:flex-row flex-col justify-start">
          ...
          <div className="sm:ml-10 sm:w-1/3 w-full">
            <label className="block text-primary text-sm">Preparation time</label>
            <select
              className="mt-1 flex w-full rounded-lg px-5 py-3 text-base text-background font-bold focus:outline-none"
              onChange={(e) => setPrepareWithin(parseInt(e.target.value))}
            >
              {[
                { minutes: 0, label: "no preference" },
                { minutes: 10, label: "<10 minutes" },
                { minutes: 30, label: "<30 minutes" },
                { minutes: 60, label: "<1 hour" },
              ].map((choice) => {
                  return <option value={choice.minutes}>{choice.label}</option>;
                })}
            </select>
          </div>
        </div>
      ...
      {response && (
        <div className="mt-10">
          <div className="mt-6 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            {response.map((recipe) => {
              if (prepareWithin == 0 || recipe.readyInMinutes < prepareWithin) {
                return (
                  ...
                  <div className="flow-root bg-light rounded-lg px-4 pb-8">
                    <div className="-mt-6">
                      ...
                      <span className="mt-2 text-sm text-secondary block">
                        ...
                        ready in minutes: {recipe.readyInMinutes}<br />
                        servings: {recipe.servings}<br />
                        source url: {recipe.sourceUrl}<br />
                        image: {recipe.image}<br />
                        compare: {recipe.readyInMinutes} and {prepareWithin}<br />
                      </span>
                          */ } 
                      </div>
      ...
      <div className="mt-20 mb-10 text-center">
        <p className="text-primary text-xs">
          Based on the tutorial by RapidAPI DevRel Team -{" "}
          <a
